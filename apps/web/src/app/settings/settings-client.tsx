...
import { Moon, Sun, Monitor, Settings, Palette, AlertTriangle, CreditCard } from 'lucide-react'
...
export default function SettingsClient({ user, subscription }: SettingsClientProps) {
  ...
  return (
    <div className="px-6 py-8 md:px-10 max-w-3xl">
      ...
      {currentTab === 'subscription' && <SubscriptionSection subscription={subscription || null} />}
...
function MobileTabNav({ currentTab }: { currentTab: string }) {
  const tabs = [
    ...
    { param: 'subscription', label: 'Plans', icon: CreditCard },
